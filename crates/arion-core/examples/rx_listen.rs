...
//!
//! ```text
//! cargo run -p arion-core --example rx_listen
//! HL2_IP=192.168.1.40 cargo run -p arion-core --example rx_listen
//! HL2_IP=192.168.1.40 cargo run -p arion-core --example rx_listen -- 14074000 USB 60
//! ```
//!
...
use arion_core::{discover, DiscoveryOptions, Radio, RadioConfig, RxConfig, WdspMode};
...
fn hpsdr_net_port() -> u16 {
    ...
    // it to `arion-core`'s re-exports but it's such a trivial number
    // that the example just hardcodes the upstream value. If that ever
    // becomes a maintenance burden, plumb it through `arion_core`.
    1024
}
