...
//!
//! ```text
//! cargo run -p arion-audio --example audio_probe
//! ```
//!
...
//! and `arion-audio::AudioOutput` are both wired correctly and we can
//! move on to [`arion-core`], which glues the radio → DSP → audio
//! pipeline together.
//!
//! Environment knobs:
//! - `AUDIO_DEVICE=<name>` — pick a non-default device. Run with
//!   `cargo run -p arion-audio --example audio_probe -- --list` to see
//!   the available names.
...
use anyhow::Result;
use arion_audio::{list_output_devices, AudioConfig, AudioOutput};
