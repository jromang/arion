//! ratatui console frontend for Thetis — rich layout matching the
...
use thetis_app::{App, AppOptions, Band, FilterPreset, AgcPreset, dbm_to_s_units, SMETER_DBFS_TO_DBM_OFFSET};
...
impl TuiView {
    ...
    fn draw_menu_bar(&self, frame: &mut Frame, area: Rect) {
        let line = Line::from(vec![
            ...
            Span::raw("  "),
            Span::styled("Thetis-tui", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]);
