//! Persistent user settings for Thetis, serialised as TOML.
//!
...
//! of any dependency on `wdsp` / `thetis-core` / `thetis-ui`. The UI
...
//!
//! On-disk layout (`thetis.toml` under `$XDG_CONFIG_HOME/thetis/`):
//!
//! ```toml
//! [general]
...
//! leave the user with a half-written `thetis.toml`.
...
/// `thetis-settings` crate doesn't need to depend on `wdsp` (which
...
impl Settings {
    ...
    pub const FILENAME: &'static str = "thetis.toml";

    /// Default config-file path: `$XDG_CONFIG_HOME/thetis/thetis.toml`
    /// on Linux, `~/Library/Application Support/thetis/thetis.toml`
    /// on macOS, `%APPDATA%\thetis\thetis.toml` on Windows.
    /// Returns `None` only on headless containers with no `HOME`.
    pub fn default_path() -> Option<PathBuf> {
        ProjectDirs::from("rs", "thetis", "thetis")
...
mod tests {
    ...
    /// (we want `thetis-settings` to stay dep-light). Removes itself
